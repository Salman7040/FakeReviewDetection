...
if (isset($_POST["submit"]) && isset($_POST["ratingPoint"]) && isset($_POST['feadback'])) {
    unset($_SESSION['repeatedReview']);
    $ipAddress = get_ip();
    $ins = "SELECT * FROM userip  where 	vivoIP='$ipAddress' ";
    $result = mysqli_query($conn, $ins);
    $res = mysqli_fetch_assoc($result);
    if (!isset($res)) {
        $query = "INSERT INTO  userip vivoIP VALUES ('$ipAddress')";
...
if ($_SESSION['GiveReview'] == "GiveReview") {
    ...
    $query = "INSERT INTO  ratingpoints vivo VALUES ('$point')";
    $data = mysqli_query($conn, $query);

    //========= user Giveable rating logic ============

    $userEmail = $_SESSION['userEmail'];
    $query = "UPDATE login SET vivo='$point' WHERE email='$userEmail'";
    $data = mysqli_query($conn, $query);

    //========= feadback logic ============
    $fedbackValue = strip_tags($_SESSION['feadback']);
    $query = "insert into  feadback vivo VALUES ('$fedbackValue')";
    $data = mysqli_query($conn, $query);
...
<head>
    ...
    <title>iphone</title>
    <link rel="stylesheet" href="iphone.css?v=11">
</head>
...
<body>
    <div id="con">
        <div id="fullCon">
            ...
            <div id="imageSlider">
                <img src="./vivo/vivo1.jpeg" alt="vivo">
            </div>
...
<script>
    ...
    let limit = 9;
    let imgFolderName = "vivo"
    let i = 1
    function right() {
        i++
        if (i >= limit)
            i = 1
        getImage.innerHTML = "<img src='./" + imgFolderName + "/vivo" + i + ".jpeg' alt='iphone'>"
    }
    function left() {
        i--
        if (i < 1)
            i = limit - 1
        getImage.innerHTML = "<img src='./"+ imgFolderName +"/vivo" + i + ".jpeg' alt='vivo'>"
